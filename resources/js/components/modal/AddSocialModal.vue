<template>
    <div class="modal fade" id="addSocialModalMessage" tabindex="-1" role="dialog" aria-labelledby="addSocialModalMessageTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="ModalLabel">Добавление аккаунта соцсети</h5>
          ...
          <div class="modal-body">
                <form>
                   <div class="form-group">
                     <label>Соцсеть</label>                        
                        <select class="form-control" name="choices-button" id="choices-button" placeholder="Выберите соцсеть" v-model="selected" v-if="socialNetworksData.value.length > 1">
                           <option v-for="item in socialNetworksData.value" :value="item.id" :key="item.id">
                              {{ item.name }}
                           </option>
                        </select>   
                        <vsud-input
                           ...
                           :isRequired="true"
                           :active="false"
                           :value="socialNetworksData.value.length ? this.socialNetworksData.value[0].name : ''"
                           :disabled="true"
        ...
        axios.get("/sanctum/csrf-cookie").then((response) => {
          axios
            .post("/api/add-social", {
              ...
              socialNetworkId: this.socialNetworksData.value.length == 1 ? this.socialNetworksData.value[0].id : this.selected,
